...
因此定义复数 $z$ 的模 $|z|$ 为其对应的向量 $\overrightarrow{OZ}$ 的模，即 $|z|=\sqrt{a^2+b^2}$。定义模为 $1$ 的复数为 **单位复数**，全体单位复数在复平面上构成 **单位圆周**。

由向量的定义，**虚数** 不可比较大小。

以上为平面直角坐标系下的分析，同理，可以使用 **极坐标系** 表示复平面，复数 $z$ 的极坐标为 $(r,\theta)$，其中 $r=|z|$。

此时定义复数 $z=a+b\rm i$ 的 **辐角** $\theta=\text{Arg } z$ 为实轴正向到复数 $z$ 对应向量的夹角，可得 $\theta$ 满足关系式为 $\theta=\tan \frac{y}{x}$。

可以发现任意非零复数有无数个辐角，$\text{Arg }z$ 实质上为一个集合，因此定义 **辐角主值（主辐角）** $\text{arg } z$ 为其中一个特定值，满足 $-\pi<\text{arg }z\leq\pi$。

因此 $\text{Arg }z=\{\text{arg }z+2k\pi|k\in\rm Z\}$。

## $\text{Operations}$ 运算

给定复数 $z_1=a_1+b_1\rm i$，$z_2=a_2+b_2\rm i$，则它们对应的向量分别为 $\overrightarrow{OZ_1}=(a_1,b_1),\overrightarrow{OZ_2}=(a_2,b_2)$，极坐标为 $(|z_1|,\theta_1),(|z_2|,\theta_2)$。
...
$$
z_1\cdot z_2 = (a_1+b_1\rm{i}\displaystyle)(a_2+b_2\rm i\displaystyle)=(a_1a_2-b_1b_2)+(a_1b_2+a_2b_1)\rm i
$$

> $\text{OI-wiki}$ 上说与向量叉乘形式类似，但二维向量叉乘 $\overrightarrow{OZ_1}\times\overrightarrow{OZ_2}=(a_1b_2-a_2b_1)\rm i$，从坐标、模长等角度均看不出相似性。

这里与坐标变化的相关性最好还是需要放到极坐标下研究，设复数相乘结果的极坐标为 $(r,\theta)$。显然有 $\sqrt{(a_1a_2-b_1b_2)^2+(a_1b_2+a_2b_1)^2}=\sqrt{a_1^2+b_1^2}\cdot\sqrt{a_2^2+b_2^2}$，因此有 $r=|z_1\cdot z_2| = |z_1|\cdot|z_2|$。
...
1. 模恒正：$|\text{exp }z|=\text{exp }x>0$。

___proof.___

$$
\begin{align*}
|\text{exp } z|&=\sqrt{(e^x\cos y)^2+(e^x\sin y)^2}\\
&=\sqrt{e^{2x}(\cos^2 y+\sin^2 y)}\\
&=\sqrt{e^{2x}}=e^x>0
\end{align*}
$$

同时有 $\text{exp }x=e^x$，因此 $|\text{exp }z|=\text{exp }x$。
